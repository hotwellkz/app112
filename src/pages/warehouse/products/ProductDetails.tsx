  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(movementRef, {
        ...
        price: product.averagePurchasePrice || 0,
        totalPrice: Math.abs(editedQuantity - product.quantity) * (product.averagePurchasePrice || 0),
        date: serverTimestamp(),
        description: 'Ручная корректировка количества',
        warehouse: 'Основной склад',
        previousQuantity: product.quantity,
        newQuantity: editedQuantity,
        previousAveragePrice: product.averagePurchasePrice || 0,
        newAveragePrice: product.averagePurchasePrice || 0
      });

      setProduct({ ...product, quantity: editedQuantity });
      setIsEditing(false);
      showSuccessNotification('Количество успешно обновлено');
    } catch (error) {
      console.error('Error updating quantity:', error);
      showErrorNotification('Ошибка при обновлении количества');
    } finally {
      setIsSaving(false);
    }
  };
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 space-y-2">
                  ...
                  <div className="text-sm text-gray-600">
                    ...
                    <span className="font-medium">{product.averagePurchasePrice?.toLocaleString()} ₸</span>
                  </div>
                  <div className="text-sm text-gray-600">
                    <span>Общая стоимость: </span>
                    <span className="font-medium">{(product.quantity * (product.averagePurchasePrice || 0)).toLocaleString()} ₸</span>
                  </div>
