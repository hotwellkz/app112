...
export const ProductHistory: React.FC<ProductHistoryProps> = ({
  ...
  const [swipedStates, setSwipedStates] = useState<{[key: string]: boolean}>({});
  const swipeableConfig = {
    trackMouse: true,
    preventScrollOnSwipe: true,
    onSwipedLeft: (e: SwipeEventData) => {
      const movementId = e.event.currentTarget.getAttribute('data-movement-id');
      if (movementId) {
        setSwipedStates(prev => ({
          ...prev,
  ...
  const { ref: swipeRef, ...swipeHandlers } = useSwipeable(swipeableConfig);
                ...
                >
                  <div 
                    className={`absolute inset-y-0 right-0 w-16 bg-red-500 flex items-center justify-center transition-opacity duration-200 ${
                      swipedStates[record.id] ? 'opacity-100' : 'opacity-0'
                    }`}
                  >
                    <button
                  ...
                  <div 
                    className={`bg-white border rounded-lg p-4 hover:shadow-sm transition-all transform ${
                      swipedStates[record.id] ? '-translate-x-16' : 'translate-x-0'
                    }`}
                  >
